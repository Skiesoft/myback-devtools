...
test('Test save object', async () => {
  s.name = 'Test name'
  s.age = 20
  await db.save<Sample>(Sample, s)
  expect((await db.all<Sample>(Sample)).length).toBe(1)
})

test('Test find object', async () => {
  let query = new QueryBuilder().greaterThan('age', '50')
  expect((await db.find<Sample>(Sample, query)).length).toBe(0)
  query = new QueryBuilder().lessOrEqualThan('age', '50')
  expect((await db.find<Sample>(Sample, query)).length).toBe(1)
})

test('Test update object', async () => {
  s.age = 30
  await db.save<Sample>(Sample, s)
  const vals = await db.all<Sample>(Sample)
  expect(vals[0].age).toBe(30)
})

test('Test delete object', async () => {
  await db.destroy<Sample>(Sample, s)
  expect((await db.all<Sample>(Sample)).length).toBe(0)
})
